#
# Be sure to run `pod lib lint unLimitPrint.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see http://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'unLimitPrint'
  s.version          = '0.1.0'
  s.summary          = 'let use unlimit print for destroy your xcode'


  s.description      = <<-DESC
let use unlimit print for destroy your xcode LOL!!!
                       DESC

  s.homepage         = 'https://github.com/zodidac/unLimitPrint'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'Natthaaek' => 'natthaaek.f@gmail.com' }
  s.source           = { :git => 'https://github.com/zodidac/unLimitPrint.git', :tag => s.version.to_s }
  # s.social_media_url = 'https://twitter.com/<TWITTER_USERNAME>'

  s.ios.deployment_target = '9.1'

  s.source_files = 'unLimitPrint/Classes/**/*'
  
  # s.resource_bundles = {
  #   'unLimitPrint' => ['unLimitPrint/Assets/*.png']
  # }

  # s.public_header_files = 'Pod/Classes/**/*.h'
   s.frameworks = 'UIKit'
  # s.dependency 'AFNetworking', '~> 2.3'
end
